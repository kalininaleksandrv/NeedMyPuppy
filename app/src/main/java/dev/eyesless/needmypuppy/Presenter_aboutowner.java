...
class Presenter_aboutowner {
    ...
    public void valuereader() {
        ...
        inact.active.setValue(max(inact.active.getValue(), max(finalyactive(), max(minterface.isexp(), minterface.istime())))); //чем больше времени, опыта или активности, тем более активная порода допускается
